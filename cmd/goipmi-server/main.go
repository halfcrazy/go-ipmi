...
func run() error {
	...
	// Add test user: ADMIN / ADMIN with Administrator privilege on LAN channel 1.
	...
	// Unblock Serve()'s blocking ReadFrom when the context is cancelled.
	go func() {
		<-ctx.Done()
		srv.Close()
	}()

	fmt.Printf("goipmi-server: listening on %s (lanplus, user=ADMIN, pass=ADMIN)\n", addr)
