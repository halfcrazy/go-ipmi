...
SERVER_BIN="${ROOT_DIR}/_output/goipmi-server"
GOIPMI="${ROOT_DIR}/_output/goipmi"

GOIPMI_SERVER_PORT="${GOIPMI_SERVER_PORT:-623}"
...
# Build binaries.
echo "==> Building goipmi-server ..."
(cd "${ROOT_DIR}" && go build -o "${SERVER_BIN}" ./cmd/goipmi-server/)
...
echo ""
echo "--- chassis status ---"
${IPMITOOL_RUN} -H 127.0.0.1 -p "${GOIPMI_SERVER_PORT}" \
	-U "${GOIPMI_USER}" -P "${GOIPMI_PASS}" -I lanplus chassis status

echo ""
echo "========================================"
echo " Server E2E: PASSED"
echo "========================================"
